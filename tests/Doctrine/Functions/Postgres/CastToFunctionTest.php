...
class CastToFunctionTest extends TestCase
{
    ...
    public function testGetSql()
    {
        ...
        $query = $this->em
            ->createQuery('SELECT a FROM Somnambulist\Components\Tests\Support\Stubs\Models\Order a WHERE ILIKE(CAST(a.name, \'text\'), :name) = true')
        ;
        $sql = $query->getSQL();
    ...
    public function testGetSqlRaisesExceptionForInvalidCast()
    {
        ...
        $query = $this->em
            ->createQuery('SELECT a FROM Somnambulist\Components\Tests\Support\Stubs\Models\Order a WHERE ILIKE(CAST(a.name, \'json\'), :name) = true')
        ;
        $query->getSQL();
    }
}
